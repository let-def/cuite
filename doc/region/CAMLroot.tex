...
\begin{itemize}
\item when \cpp{mk\_pair\_c} is applied, the OCaml compiler generates a
      call to \cpp{mk\_pair\_c\_impl} symbol,
\item a C file must define a function of this name,
\item at link time, the system linker connects the two together.
\end{itemize}
...
\paragraph{CAMLparam} \cpp{CAMLparam2(a,b)} expands to two other macros:
...
implemented by the \cpp{struct\ caml\_\_roots\_block} type, and
...
The last macro is \cpp{CAMLreturn}. It restores the previous set of local
roots by setting the \cpp{caml\_local\_roots} variable to the state saved by
\cpp{CAMLparam0();}.
...
Armed with \cpp{mk\_pair\_c\_impl} and following the rules above, one
...
observation is that any well-formed argument of type \cpp{value*} is
...
The main API function that seems to break the \cpp{value*} rule is
\cpp{mlroot\_get\_field}, which returns a value. But that is precisely
